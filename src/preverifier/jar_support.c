...
bool JARfile = false;    /* if true, indicates that output is in a
                                                  JAR file */
extern bool tmpDirExists;
...
loadJARfile(zip_t *entry, const char* filename)
{
    ...
    unsigned long decompLen = 0; /* the decompressed length */
    unsigned long compLen; /* the compressed length */
    unsigned long method; /* how it is stored */
    unsigned long expectedCRC; /* expected CRC */
    unsigned long actualCRC;

    unsigned char *decompData = NULL;
    if (file == NULL) {
        goto done;
    }
    ...
    decompLen = CENLEN(p); /* the decompressed length */
    compLen   = CENSIZ(p); /* the compressed length */
    method    = CENHOW(p); /* how it is stored */
    expectedCRC = CENCRC(p); /* expected CRC */
 ...
 *   returns:     boolean type
 *=======================================================================*/
bool ProcessJARfile(char *buf, int len)  {
    ...
    if (findJARDirectories(zipEntry, &stat_buf)) {
       ...
       if (JARfile && tmpDirExists && output_dir) {
           const char *p;
