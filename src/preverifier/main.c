...
bool no_native_methods = false;
bool no_floating_point = false;
bool no_finalizers = false;
...
extern bool ProcessJARfile(char *buf, int len);
extern bool isJARfile(char *fn, int length);
...
int main(int argc, char *argv[])
{
    ...
    if ((++argv)[0][0] == '-') {
        ...
        verify_verbose++;
                ...
                extern bool stack_map_on;
                stack_map_on = false;
            } else if (strcmp(argv[0], "-Xni") == 0) {
                extern bool inline_jsr_on;
                inline_jsr_on = false;
